#!/usr/bin/env python3
"""Linux SITL reboot regression: python3 test/test_sitl_reboot.py [path/to/px4].
...
def run(binary):
    ...
    with tempfile.TemporaryDirectory(prefix="sitl-reboot-") as directory:
        ...
        work = root / "rootfs"
        work.mkdir()
        mav = mavutil.mavlink_connection("udpin:127.0.0.1:0", source_system=250)
        ...
        (work / "rcS").write_text(
            ...
            "dataman start\n"
            "commander start\n"
            f"mavlink start -u {unused_port()} -o {receive_port} -t 127.0.0.1\n"
            "mavlink boot_complete\n"
            "echo boot >> boots\n"
        )
        with (root / "px4.log").open("w+") as log:
            # Relative executable, working directory and script exercise launch-context restoration.
            ...
            def client(command, *args):
                return subprocess.check_output(
                    [str(binary.with_name("px4-" + command)), "--instance", str(instance), *args],
            ...
            try:
                ...
                # Force-arm only this isolated test instance to exercise the armed guard.
                arm = mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM
                client("commander", "arm", "-f")
                assert heartbeat().base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED
                assert command(reboot, 1) == mavutil.mavlink.MAV_RESULT_DENIED
                assert command(arm, 0, 21196) == mavutil.mavlink.MAV_RESULT_ACCEPTED
                ...
                for boot in range(2, 5):
                    if boot == 4:
                        # An open MAVLink console redirects process stdout/stderr.
                        data = b"ver all\n"
                        ...
                        assert mav.recv_match(type="SERIAL_CONTROL", blocking=True, timeout=5)
                    assert command(reboot, 1) == mavutil.mavlink.MAV_RESULT_ACCEPTED
                    deadline = time.monotonic() + 20
                    while len((work / "boots").read_text().splitlines()) < boot:
                        assert process.poll() is None, "PX4 exited instead of restarting"
                        assert time.monotonic() < deadline, "PX4 did not restart"
                        time.sleep(0.1)
                    assert not heartbeat().base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED
                    assert float(client("param", "show", "-q", "COM_RC_LOSS_T")) == 9
                    assert len(list(Path(f"/proc/{process.pid}/fd").iterdir())) <= fd_count
                    print(f"PASS: boot {boot}, same PID, parameters retained, MAVLink reconnected")

                log.seek(0)
                assert log.read().count("px4 starting.") == 4, "Console output was not restored"
                assert command(reboot, 2) == mavutil.mavlink.MAV_RESULT_ACCEPTED
                assert process.wait(timeout=10) == 0
                print("PASS: bootloader/armed reboot denied; ordinary shutdown still exits")
...
if __name__ == "__main__":
    run(Path(sys.argv[1] if len(sys.argv) > 1 else "build/px4_sitl_default/bin/px4").resolve())
